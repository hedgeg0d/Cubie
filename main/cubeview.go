...
func NewCubeView(model *cubestate.Model) *CubeView {
	c := &CubeView{model: model, yaw: 0.7, pitch: 0.5}
...
func (r *cubeViewRenderer) Layout(s fyne.Size)        { r.img.Resize(s) }
func (r *cubeViewRenderer) MinSize() fyne.Size        { return fyne.NewSize(300, 300) }
...
func (c *CubeView) render() image.Image {
	...
	img := image.NewRGBA(image.Rect(0, 0, cubeViewSize, cubeViewSize))
	for i := range img.Pix {
		img.Pix[i] = 0
	}
	for x := 0; x < cubeViewSize; x++ {
		for y := 0; y < cubeViewSize; y++ {
			img.SetRGBA(x, y, color.RGBA{24, 24, 28, 255})
		}
	}
	if model == nil {
		return img
	...
	for _, q := range quads {
		fillQuad(img, q.pts, q.col)
		strokeQuad(img, q.pts, color.RGBA{15, 15, 15, 255})
	}
	return img
}
...
func fillQuad(img *image.RGBA, pts [4][2]float64, col color.RGBA) {
	...
	for y := y0; y <= y1; y++ {
		for x := x0; x <= x1; x++ {
			if x < 0 || y < 0 || x >= cubeViewSize || y >= cubeViewSize {
				continue
			}
...
func strokeQuad(img *image.RGBA, pts [4][2]float64, col color.RGBA) {
	for i := 0; i < 4; i++ {
		a := pts[i]
		b := pts[(i+1)%4]
...
func drawLine(img *image.RGBA, x0, y0, x1, y1 int, col color.RGBA) {
	...
	err := dx + dy
	for {
		if x0 >= 0 && y0 >= 0 && x0 < cubeViewSize && y0 < cubeViewSize {
