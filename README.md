...
- **Lettering** — assign a memo symbol to every non-center sticker on a flat cube net; click a sticker and type (any alphabet, e.g. Cyrillic). Symbols update live on the net. Multiple named schemes are saved as profiles (defaults to Speffz) in `lettering_profiles.json`.
